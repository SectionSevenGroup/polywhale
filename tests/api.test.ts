...
test("detail APIs never include demo fallback and expose explicit safe states",()=>{const signal=fs.readFileSync("app/api/signals/[id]/route.ts","utf8"),whale=fs.readFileSync("app/api/whales/[wallet]/route.ts","utf8");assert.doesNotMatch(signal+whale,/demo|mock|fixture/i);assert.match(signal,/available:false,points:\[\],reason:/);assert.match(signal,/Signal not found/);assert.match(whale,/Tracked whale not found/);assert.match(signal,/signal_events\?/);assert.match(signal,/fetchPriceHistory/)});
