...
export async function GET(_:Request,{params}:{params:Promise<{id:string}>}) {
  try {
    ...
    try {
      ...
      return Response.json({signal,trades,events,priceHistory:{available:history.length>0,points:history,reason:history.length?null:"No historical prices returned"}});
    } catch {
      return Response.json({signal,trades,events,priceHistory:{available:false,points:[],reason:"Historical price data is currently unavailable"}});
    }
